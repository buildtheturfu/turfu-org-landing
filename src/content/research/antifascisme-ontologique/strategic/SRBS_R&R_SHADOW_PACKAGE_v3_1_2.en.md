# SRBS R&R Shadow Package — B1 v3.1.2
...
|---|---:|---|
| Desk reject | 15-25% | si éditeur juge politique trop spéculatif pour SRBS |
...
## Module A — Terminology Softening : "structural realization" → "candidate"

### Prédiction reviewer (~80-90% probabilité de demande)
...
Adoucir hors biologie. Garder "instantiated biologically" pour cancer (formellement le plus solide), passer politique + écologie en "candidate structural realizations" ou "domain-level readings".
...
## Module B — Worked Reichsfluchtsteuer Mapping (S, ∂S, φ, f, e)

### Prédiction reviewer (~70-80% probabilité)
...
UN seul mapping détaillé (Reichsfluchtsteuer = le mieux sourcé). Les autres cas restent "analogous domain readings" non-formalisées. Ne PAS faire les trois cas — surformalisation = nouvelle prise pour attaque "where is the category of social agents?"
...
## Module C — Normative Neutrality Reinforcement

### Prédiction reviewer (~70-85% probabilité)
...
Ne pas se battre sur l'étiquette libertarienne. Reformuler : "framework constrains, does not entail." Renforcer la parité des 4 completions (property/NAP, Ostrom, Habermas, Pettit). Déplacer une partie du langage propriété/non-agression du §5.3 vers §6.

### Patches précis

**C.1 — §5.4 (ligne 159, déjà titré "(one reading among several)")**
...
Insert (nouvelle phrase de clôture du paragraphe d'intro) :
...
## Module D — Empirical Status Reformulation : "hypothesis-generating, not confirmatory"

### Prédiction reviewer (~85-95% probabilité)

> "Three cases do not establish the empirical thesis. What is the epistemic status of these illustrations?"

### Stratégie

Accepter sans résistance. Le papier anticipe déjà ce point (§4.2 méthodologique caveat, §6.2 limit). Renforcer explicitement avec "plausibility probes / hypothesis-generating, not confirmatory."
...
Add at end of §4 opening paragraph :
...
## Module E — Boundary Conditions of In Situ Exit

### Prédiction reviewer (~80-90% probabilité)
...
Le papier reconnaît déjà la limite (ligne 137 : "in any system with shared infrastructure... in situ exit cannot be absolute — rival goods and infrastructural dependencies generate residual coupling"). Renforcer avec mini-section dédiée donnant exemple positif (protocol fork, data portability, DAO ragequit) + contre-exemple physique (land, fragile public goods).

### Patch précis — nouvelle micro-section §5.3.1 à insérer après ligne 137
...
## Module F — "Diagnostic, not theorematic" front-loaded

### Prédiction reviewer (~90% probabilité)
...
## Module G — Ashby/Set Note (probabilité très faible mais peu coûteux)

### Patch précis — note de bas de page ou phrase incise en §3 (après ligne 59)

Add after "This connects formally to Ashby's (1956) law of requisite variety" :
...
| Reviewer pattern | Modules à déployer | Note |
|---|---|---|
| **Major revision constructive standard** (Peter G scénario principal) | A + B + C + D + F | 5 modules, ~2 semaines de R&R |
...
| **Reviewer hostile idéologique** (libertarian smuggling) | C en priorité, accompagné A et D | Cover letter R&R doit insister sur les 4 completions à parité |
...
## Probability conversion (after deploying full Module A-G stack)

| Initial outcome | Conversion after R&R | Final P(acceptance) |
|---|---:|---:|
| Major revision constructive | 60-70% | ~0.40 × 0.65 = **0.26** |
| Major revision hostile/detailed | 35-50% | ~0.05 × 0.42 = **0.02** |
| Reject → resubmit B2/Cosmos | — | papier recyclable |

Estimation totale acceptance SRBS finale, conditionnel sur passage du desk : **~25-35%**, conforme à Peter G.
...
- **Plus tu attends, plus le reviewer est froid** : viser réponse en 2-3 semaines pour signaler engagement constructif
- **Cover letter R&R** : 1 page max, lister les 3-5 changements majeurs en bullets
- **Track changes** : Wiley exige version avec changes visibles + version clean
...
|---|---|---|
| A — Terminology softening | ✅ Patches prêts | 2026-05-30 |
| B — Reichsfluchtsteuer mapping | ✅ Bloc rédigé | 2026-05-30 |
| C — Normative neutrality | ✅ Patches prêts | 2026-05-30 |
| D — Empirical status | ✅ Patches prêts | 2026-05-30 |
| E — Boundary conditions in situ | ✅ Section rédigée | 2026-05-30 |
| F — Diagnostic-not-theorematic | ✅ Phrase prête | 2026-05-30 |
| G — Ashby/Set note | ✅ Note prête | 2026-05-30 |

**À déployer dès réception reviewer reports**.
